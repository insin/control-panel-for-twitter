...
async function tweakSearchIcon() {
  let $searchIconPath = await getElement(`svg path[d="${Svgs.X_SEARCH_PATH}"]`, {name: 'Search icon', stopIf: pageIsNot(currentPage)})
  if ($searchIconPath) {
    // Safari doesn't support using `d: path(…)` to replace paths in an SVG, so
    // we have to manually patch the path in it.
    $searchIconPath.setAttribute('d', Svgs.TWITTER_SEARCH_PATH)
  }
}
